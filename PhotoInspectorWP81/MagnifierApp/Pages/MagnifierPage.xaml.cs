...
namespace MagnifierApp
{
    public partial class MagnifierPage : PhoneApplicationPage
    {
        ...
        {
            if (e.PropertyName == PhotoModel.PathPropertyName)
            {
            ...
            _saveButton.IsEnabled = !PhotoModel.Singleton.IsSaved();
                
            // Enable revert to original button if there is an original to revert to
            _revertMenuItem.IsEnabled = PhotoModel.Singleton.IsModified();
        }
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (e.NavigationMode == NavigationMode.New)
            {
                ...
                if (token != null)
                {
                    await PhotoModel.Singleton.FromToken(token);
                }
            }
        ...
        {
            if (PhotoModel.Singleton.IsSaved())
            {
                if (PhotoModel.Singleton.Path.Contains(KnownFolders.CameraRoll.Path))
                {
                    InformationTextBlock.Text = AppResources.MagnifierPage_InformationTextBlock_CameraRollText;
                }
                else
                {
                    InformationTextBlock.Text = AppResources.MagnifierPage_InformationTextBlock_SavedPicturesText;
                }
            }
            else
            {
        ...
        private async void ShareButton_Click(object sender, EventArgs e)
        {
            if (!_saving)
            {
                ...
                _shareMediaTask.FilePath = PhotoModel.Singleton.Path;
        ...
        private async void PhotoChooserTask_Completed(object sender, PhotoResult e)
        {
            if (e.TaskResult == TaskResult.OK)
            {
                ...
                {
                    await PhotoModel.Singleton.FromLibraryPath(e.OriginalFileName);
